...
The repository utilizes a modular, lightweight structure designed to separate experimental training environments from the live client application state:

Plaintext
Smart_Home_Energy_Predictor/
├── app.py                         # Interactive Streamlit frontend UI controller
├── energy_regression_model.pkl    # Serialized algorithmic regression model weights
├── model_features.pkl             # Serialized metadata map for slider column alignment
└── home_energy_predictor.ipynb    # Core data analytics & machine learning training noteb
